...
namespace WebFontClient.SDK
{
    public class WebFontClient
    {
        ...
        /// </summary>
        private readonly string _address = "http://localhost:5000";

        /// <summary>
        /// 需替换用户的apiKey(apiKey是由docker-compose.yml里的WEBFONT_APIKEY定义，两者必须对应才能调用成功)
        /// </summary>
        private readonly string _apiKey = "5c051ef2-441f-4ed2-8f3b-61506da724e9";
        #endregion        

        #region 构造函数        
        public WebFontClient() : this("", "")
        {
            
        }

        public WebFontClient(string address) : this(address, "")
        {

        }

        public WebFontClient(string address, string apikey)
        {
        ...
        /// <returns></returns>
        private WebFontInfo BuildFont(string apikey, string accessKey, string content)
        {
            FontBuildAccessKeyRequest request = new FontBuildAccessKeyRequest
            {
                ...
                NeedEot = true, //如果不需要此格式的文件返回，则此处参数设置为false
                NeedWoff = true, //如果不需要此格式的文件返回，则此处参数设置为false
                NeedWoff2 = true //如果不需要此格式的文件返回，则此处参数设置为false
            };

            try
            {
                var response = _client.BuildFontForAccessKey(request);
        ...
        /// <param name="content">需要处理的文本</param>
        /// <returns></returns>
        private List<WebFontInfo> Multbuild(string apiKey, int[] fontIds, string content)
        {
            ...
            try
            {
                var response = _client.MultBuildFont(buildRequest);
        ...
        {            
            ...
            try
            {
                ...
                if (exists)
                {
                    ...
                    var expiration = DateTime.Now.AddDays(1000);             
        ...
        public WebFontFace GetFontFace(FontFaceParam param)
        {
            ...
            WebFontInfo webfontinfo = BuildFont(_apiKey, param.accessKey, param.content);
